...
Inline F3 image_sample_bilinear(Image image, F1 u, F1 v) { 
	u = F1_clamp01(u);
	v = F1_clamp01(v);
...
Internal void lane(L1 arena) {
	...
	if (lane_idx() == 0) {
		...
		ramR->lens_dirt_image = image_read_from_file(arena, "lens-dirt.bmp");
	}
	...
	if (lane_idx() == 0) {
		...
		for EachIndex(y, output_height) {
			for EachIndex(x, output_width) {
				...
				F3 dirt_color = image_sample_bilinear(ramR->lens_dirt_image, u, v);
				F1 dirt_luminance = F3_luminance(dirt_color);

				F3 ray_color = TR_(F3, in_ray)[0];
				F3 bloom_color = TR_(F3, in_bloom)[0];
				F3 color = ray_color * (1.0f - 0.5f*bloom_strength) +
									 bloom_strength*bloom_color * (0.5f + 0.5f*dirt_luminance);
