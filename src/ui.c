...
Internal UI_Signal ui_drag_F1(F1 *value, F1 default_value, F1 pixels_per_unit, F1 min, F1 max) {
  ...
  if (min != 0) value[0] = Max(min, value[0]);
  if (max != 0) value[0] = Min(max, value[0]);
...
Internal UI_Signal ui_drag_L1(L1 *value, L1 default_value, L1 pixels_per_unit, L1 min, L1 max) {
  String8 key_string = str8f(ui_build_arena(), "drag_%p", value);
  UI_Key key = ui_key_from_string(ui_active_seed_key(), key_string);
  UI_Box *box = ui_build_box_from_key(UI_BOX_FLAG__MOUSE_CLICKABLE |
                                      UI_BOX_FLAG__DRAW_BACKGROUND |
  ...
  if (pixels_per_unit == 0.0f) {
    ...
    // auto calculate pixels per unit when possible
    if ((min != 0 || max != 0) && max > min && max < L1_MAX && signal.box->rect[2] > 0.0f) {
      L1 range = max - min;
      pixels_per_unit = (F1)((D1)signal.box->rect[2] / (D1)range);
    }
  }
  ...
  if (signal.flags & UI_SIGNAL_FLAG__LEFT_DRAGGING) {
    ...
    L1 drag_max = max != 0 ? max : L1_MAX;
    initial_value = Clamp(min, initial_value, drag_max);
    if (delta < 0) {
      L1 magnitude = (L1)-delta;
      value[0] = magnitude > initial_value - min ? min : initial_value - magnitude;
    } else {
      L1 magnitude = (L1)delta;
      value[0] = magnitude > drag_max - initial_value ? drag_max : initial_value + magnitude;
    }
  }
...
Internal UI_Signal ui_drag_L1_label(String8 string, CString format, L1 *value, L1 default_value, F1 pixels_per_unit, L1 min, L1 max) {
  UI_Signal signal = ui_drag_L1(value, default_value, pixels_per_unit, min, max);
  UI_Parent(signal.box) {
    ui_set_next_pref_width(ui_pct(1.0f, 0.0f));
    ui_label(str8f(ui_build_arena(), format, (int)string.len, string.str, value[0]));
  }
